...
def preprocess_csv(file_path: str,
    ...
    # 明确类别型并 one-hot：RatecodeID / payment_type / PULocationID / DOLocationID
    ...
    df = pd.get_dummies(df,
                        columns=['RatecodeID', 'payment_type', 'PULocationID', 'DOLocationID'],
                        prefix=['Ratecode', 'paytype', 'PU', 'DO'],
                        dummy_na=False)
    print(f"特征 one-hot 完成：当前特征数 {df.shape[1]}", flush=True)
